...
//
//  Modify by Luisa Cruz on 02/04/2015.
//
...
public class CSVReader {
    ...
    init(string: String) {
        var csv = string.stringByReplacingOccurrencesOfString("\r", withString: "", options: NSStringCompareOptions.LiteralSearch, range: nil)
        processLines(csv)
        _numberOfColumns = _lines[0].componentsSeparatedByString("\t").count
        _numberOfRows = _lines.count - 1
        headers = _lines[0].componentsSeparatedByString("\t")
